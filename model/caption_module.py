...
class TopDownSceneCaptionModule(nn.Module):
    ...
    def forward(self, data_dict, use_tf=True, is_eval=False, max_len=CONF.TRAIN.MAX_DES_LEN, stochastic=False):
        if not is_eval:
            data_dict = self._forward_sample_batch(data_dict, max_len)
        else:
            data_dict = self._forward_scene_batch(data_dict, use_tf, max_len, stochastic=stochastic)
    ...
    def _forward_scene_batch(self, data_dict, use_tf=False, max_len=CONF.TRAIN.MAX_DES_LEN, stochastic=False):
        """
        generate descriptions based on input tokens and object features
        """
        ...
        for prop_id in range(self.num_proposals):
            ...
            while True:
                ...
                step_probs = []
                for batch_id in range(batch_size):
                    # step_output[batch_id, 1] = 0
                    probabilities = torch.nn.functional.softmax(step_output[batch_id])
                    if stochastic:
                        idx = np.random.choice(self.num_vocabs, p=probabilities.cpu().detach().numpy()) # 0 ~ num_vocabs
                    ...
                    step_actions.append(idx)
                    step_probs.append(prob)

                step_preds = torch.cat(step_preds, dim=0) # batch_size, emb_size

                step_actions = torch.stack(step_actions, dim=0)  # batch_size
                step_probs = torch.stack(step_probs, dim=0)  # batch_size
                ...
                step_actions = step_actions.unsqueeze(1) # batch_size, 1
                actions.append(step_actions)

                step_probs = step_probs.unsqueeze(1) # batch_size, 1
            ...
            actions = torch.cat(actions, dim = 1).unsqueeze(1)
            action_output.append(actions)

            probs = torch.cat(probs, dim = 1).unsqueeze(1)
            prob_output.append(probs)
        ...
        action_output = torch.cat(action_output, dim=1)  # batch_size, num_proposals, num_words - 1/max_len
        prob_output = torch.cat(prob_output, dim=1)  # batch_size, num_proposals, num_words - 1/max_len
        ...
        data_dict["actions"] = action_output
        data_dict["action_probs"] = prob_output

        return data_dict


class CapPolicy(nn.Module):
    def __init__(self, vocabulary, embeddings, emb_size=300, feat_size=128, hidden_size=512, num_proposals=256):
        super().__init__() 
    ...
    def forward(self, data_dict, max_len=CONF.TRAIN.MAX_DES_LEN):
        #data_dict = self.forward_sample_batch(data_dict, max_len)
        data_dict = self.forward_scene_batch(data_dict, max_len)
        return data_dict
    ...
    def forward_sample_batch(self, data_dict, max_len=CONF.TRAIN.MAX_DES_LEN, min_iou=CONF.TRAIN.MIN_IOU_THRESHOLD):
        ...
        step_id = 0
        step_input = word_embs[:, step_id] # batch_size, emb_size
        actions = torch.zeros(batch_size, max_len, requires_grad = True).cuda() # batch_size, max_len
        probs = torch.zeros(batch_size, max_len, requires_grad = True).cuda() # batch_size, max_len
        while True:
            # feed
            step_output, hidden = self.step(step_input, hidden)
            step_output = self.classifier(step_output) # batch_size, num_vocabs
            step_output = self.softmax(step_output)

            # select word for prediction
            for i in range(batch_size):
                action = np.random.choice(self.num_vocabs, p=step_output.detach().cpu().numpy()[i])
                prob = step_output[i,action]
                action = torch.tensor(action)
                actions[i, step_id] = action
                probs[i, step_id] = prob
    ...
    def forward_scene_batch(self, data_dict, use_tf=False, max_len=CONF.TRAIN.MAX_DES_LEN):
        ...
        for prop_id in range(self.num_proposals):
            ...
            while True:
                ...
                step_output, hidden = self.step(step_input, hidden)
                step_output = self.classifier(step_output) # batch_size, num_vocabs
                step_output = self.softmax(step_output) # batch_size, num_vocabs
                
                # predicted word
                ###CHOOSE WORD RANDOMLY FOR REINFORCE###
                step_preds = []
                step_actions = []
                step_probs = []
                for batch_id in range(batch_size):
                    idx = np.random.choice(self.num_vocabs, p = step_output[batch_id].cpu().detach().numpy()) # 0 ~ num_vocabs
                    idx = torch.tensor(idx).cuda()
                    word = self.vocabulary["idx2word"][str(idx.item())]
                    emb = torch.FloatTensor(self.embeddings[word]).unsqueeze(0).cuda() # 1, emb_size
                    prob = step_output[batch_id][idx]
                    step_preds.append(emb)
                    step_actions.append(idx)
                    step_probs.append(prob)
