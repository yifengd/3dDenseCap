...
def collate(data_dicts):
    default_batch = default_collate(data_dicts)
    batch = pointgroup_collate(data_dicts, default_batch,
                               full_scale=CONF.PG.FULL_SCALE,
                               scale=CONF.PG.SCALE,
                               batch_size=len(data_dicts),
                               max_npoint=CONF.MAX_N_POINT)  # values from pointgroup config/ defaults
    batch['batch_size'] = torch.tensor(len(data_dicts))
    return batch


def pointgroup_collate(data_dicts, default_batch, full_scale, scale, batch_size, max_npoint, mode=4):
    locs = []
    locs_float = []
    feats = []
    labels = []
    ...
    for i, data_dict in enumerate(data_dicts):
        xyz_origin = data_dict["point_clouds"][:, 0:3]

        # Zero-center
        xyz_origin = xyz_origin[:, :3] - xyz_origin[:, :3].mean(0)

        rgb_and_feats = data_dict["point_clouds"][:, 3:]

        label = data_dict['semantic_labels']
        instance_label = data_dict['instance_labels']
        ...
        ### crop
        xyz, valid_idxs = crop(xyz, full_scale=full_scale, max_npoint=max_npoint)

        xyz_middle = xyz_middle[valid_idxs]
        xyz = xyz[valid_idxs]
        rgb_and_feats = rgb_and_feats[valid_idxs]
        ...
        feats.append(torch.from_numpy(rgb_and_feats) + torch.randn(rgb_and_feats.shape[1]) * 0.1)
    ...
    ### voxelize
    voxel_locs, p2v_map, v2p_map = pointgroup_ops.voxelization_idx(locs, batch_size, mode)

    return {'locs': locs, 'voxel_locs': voxel_locs, 'p2v_map': p2v_map.long(), 'v2p_map': v2p_map,
            'locs_float': locs_float, 'feats': feats,
            'pg_semantic_labels': labels,
            'pg_instance_labels': instance_labels,
            'instance_info': instance_infos,
            'instance_pointnum': instance_pointnum, # Instance infos, like bounding boxes
            # 'id': id,
            'batch_offsets': batch_offsets,
            'spatial_shape': torch.tensor(spatial_shape), **default_batch}
