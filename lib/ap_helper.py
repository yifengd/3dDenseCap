...
from model.capnet import separate_into_batches

sys.path.append(os.path.join(os.getcwd(), "lib")) # HACK add the lib folder
from util.eval_det import eval_det_cls, eval_det_multiprocessing
from util.eval_det import get_iou_obb
...
def gt_scores(data_dict, bsize, num_proposals) -> Tensor:
    ## GT-Scores
    from lib.loss_helper import get_segmented_scores
    ...
    batch_offsets = data_dict['batch_offsets']
    obj_logits = separate_into_batches(bsize, proposals_idx, proposals_offset, batch_offsets,
                                                         gt_scores.squeeze(), num_proposals).cuda()

    return obj_logits

def parse_predictions(end_points, config_dict, use_gt_scores=False):
    ...
    """
    pred_center = end_points['bbox_parameters'][:, :, :3] # B,num_proposal,3
    ...
    pred_sem_cls = end_points['bbox_sems'] # B,num_proposal
    # sem_cls_probs = softmax(end_points['bbox_sems'].detach().cpu().numpy()) # B,num_proposal,10
    ...
    for i in range(bsize):
        for j in range(num_proposal):
            ...
            corners_3d_upright_camera = end_points['bbox_corner'][i, j, :, :]
            pred_corners_3d_upright_camera[i,j] = corners_3d_upright_camera.cpu().detach()
    ...
    if use_gt_scores:
        obj_prob = gt_scores(end_points, bsize, K)
    else:
        obj_prob = end_points['bbox_scores'].detach().cpu().numpy()

    # Debug reporting:
    # torch.set_printoptions(profile="full")
    # pred = end_points['bbox_scores'].detach()
    # pred_masked = pred[pred > 0]
    # gt = gt_scores(end_points, bsize, K)[pred > 0]
    # print("PREDICTED", pred_masked)
    # print("GROUND TRUTH", gt)
    # print("BCE", torch.nn.BCELoss(reduction="mean")(pred_masked, gt))
