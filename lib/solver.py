...
from util.eta import decode_eta
...
[loss] train_pg_semantic_loss: {train_pg_semantic_loss}
[loss] train_pg_offset_norm_loss: {train_pg_offset_norm_loss}
[loss] train_pg_offset_dir_loss: {train_pg_offset_dir_loss}
[loss] train_pg_score_loss: {train_pg_score_loss}
[loss] train_pg_total_loss: {train_pg_total_loss}
...
def extract_losses(data_dict, out_dict):
    # store loss
    out_dict["cap_loss"] = data_dict["cap_loss"]
    out_dict["ori_loss"] = data_dict["ori_loss"]
    out_dict["dist_loss"] = data_dict["dist_loss"]
    out_dict["pg_semantic_loss"] = data_dict["pg_semantic_loss"]
    out_dict["pg_offset_norm_loss"] = data_dict["pg_offset_norm_loss"]
    out_dict["pg_offset_dir_loss"] = data_dict["pg_offset_dir_loss"]
    out_dict["pg_score_loss"] = data_dict.get("pg_score_loss", torch.tensor([0]))
    out_dict["pg_total_loss"] = data_dict["pg_total_loss"]
    out_dict["loss"] = data_dict["loss"]

    # store eval
    out_dict["cap_acc"] = data_dict["cap_acc"]
    out_dict["ori_acc"] = data_dict["ori_acc"]
    out_dict["pred_ious"] = data_dict["pred_ious"]
    # out_dict["obj_acc"] = data_dict["obj_acc"]
...
class Solver():
    ...
    criterion="meteor", checkpoint_best=None, train_eval_limit=None):
    ...
    def _dump_log(self, phase, is_eval=False):
        log = {
            "loss": ["loss", "cap_loss", "ori_loss", "dist_loss", "pg_semantic_loss", "pg_offset_norm_loss", "pg_offset_dir_loss", "pg_score_loss"],
            "score": ["cap_acc", "ori_acc", "pred_ious"]
        }
        for key in log:
            for item in log[key]:
                if self.log[phase][item]:
                    if (item == "cap_loss" or item == "cap_acc")\
                            and np.mean([v for v in self.log[phase][item]]) == 0:
                        continue
    ...
    def _compute_loss(self, data_dict):
        data_dict = get_scene_cap_loss(
            ...
            prepare_epochs=self.model.pointgroup.prepare_epochs
        )

        extract_losses(data_dict, self._running_log)


    def _eval(self, phase, epoch_id, samples_limit):
        if self.caption and epoch_id > self.model.pointgroup.prepare_epochs:
            bleu, cider, rouge, meteor, log = eval_cap(
                epoch_id=epoch_id,
                model=self.model,
                device=self.device,
            ...
            self.log[phase]["loss"].extend(log["loss"])
            self.log[phase]["cap_loss"].extend(log["cap_loss"])
            self.log[phase]["ori_loss"].extend(log["ori_loss"])
            self.log[phase]["dist_loss"].extend(log["dist_loss"])
            self.log[phase]["pg_semantic_loss"].extend(log["pg_semantic_loss"])
            self.log[phase]["pg_offset_norm_loss"].extend(log["pg_offset_norm_loss"])
            self.log[phase]["pg_offset_dir_loss"].extend(log["pg_offset_dir_loss"])
            self.log[phase]["pg_score_loss"].extend(log["pg_score_loss"])
            self.log[phase]["pg_total_loss"].extend(log["pg_total_loss"])

            # self.log[phase]["lang_acc"].extend(log["lang_acc"])
            self.log[phase]["cap_acc"].extend(log["cap_acc"])
            self.log[phase]["ori_acc"].extend(log["ori_acc"])
            # self.log[phase]["obj_acc"].append(self._running_log["obj_acc"])
            self.log[phase]["pred_ious"].extend(log["pred_ious"])
    ...
    def _feed(self, dataloader, phase, epoch_id, is_eval=False):
        ...
        if not is_eval:
            for data_dict in dataloader:
                data_dict['epoch'] = torch.tensor(epoch_id)
                # move to cuda
                ...
                self._running_log = {
                    ...
                    "dist_loss": 0,
                    "pg_semantic_loss": 0,
                    "pg_offset_norm_loss": 0,
                    "pg_offset_dir_loss": 0,
                    "pg_score_loss": 0,
                    "pg_total_loss": 0,
                    # acc
                    # "lang_acc": 0,
                ...
                self.log[phase]["pg_semantic_loss"].append(self._running_log["pg_semantic_loss"].item())
                self.log[phase]["pg_offset_norm_loss"].append(self._running_log["pg_offset_norm_loss"].item())
                self.log[phase]["pg_offset_dir_loss"].append(self._running_log["pg_offset_dir_loss"].item())
                self.log[phase]["pg_score_loss"].append(self._running_log["pg_score_loss"].item())
                self.log[phase]["pg_total_loss"].append(self._running_log["pg_total_loss"].item())


                # self.log[phase]["lang_acc"].append(self._running_log["lang_acc"].item())
                self.log[phase]["cap_acc"].append(self._running_log["cap_acc"].item())
                self.log[phase]["ori_acc"].append(self._running_log["ori_acc"].item())
                # self.log[phase]["obj_acc"].append(self._running_log["obj_acc"])
                self.log[phase]["pred_ious"].append(self._running_log["pred_ious"].item())
                ...
                if phase == "train":
                    ...
                    if self._global_iter_id % 100 == 0 and self._global_iter_id != 0:
                        self._reset_log("train")
                    self._global_iter_id += 1
        else:
            if phase == 'train':
                limit = self.train_eval_limit
            else:
                limit = None
            self._eval(phase, epoch_id, limit)
    ...
    def _train_report(self, epoch_id):
        ...
        iter_report = self.__iter_report_template.format(
            ...
            train_pg_semantic_loss=round(np.mean([v for v in self.log["train"]["pg_semantic_loss"]]), 5),
            train_pg_offset_norm_loss=round(np.mean([v for v in self.log["train"]["pg_offset_norm_loss"]]), 5),
            train_pg_offset_dir_loss=round(np.mean([v for v in self.log["train"]["pg_offset_dir_loss"]]), 5),
            train_pg_score_loss=round(np.mean([v for v in self.log["train"]["pg_score_loss"]]), 5),
            train_pg_total_loss=round(np.mean([v for v in self.log["train"]["pg_total_loss"]]), 5),
