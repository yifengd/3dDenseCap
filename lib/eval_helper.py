...
def decode_detections(data_dict):
    ...
    pred_heading_residual = torch.gather(data_dict['heading_residuals'], 2,
                                         pred_heading_class.unsqueeze(-1)).detach().cpu().numpy()  # B,num_proposal
    ...
    for batch_id in range(batch_size):
        ...
        for bbox_id in range(num_bbox):
            pred_obb = DC.param2obb(pred_center[batch_id, bbox_id], pred_heading_class[batch_id, bbox_id],
                                    pred_heading_residual[batch_id, bbox_id],
...
def decode_targets(data_dict):
    ...
    # assign
    pred_center = torch.gather(pred_center, 1,
                               data_dict["object_assignment"].unsqueeze(2).repeat(1, 1, 3)).detach().cpu().numpy()
    pred_heading_class = torch.gather(pred_heading_class, 1, data_dict["object_assignment"]).detach().cpu().numpy()
    pred_heading_residual = torch.gather(pred_heading_residual, 1, data_dict["object_assignment"]).unsqueeze(
        -1).detach().cpu().numpy()
    pred_size_class = torch.gather(pred_size_class, 1, data_dict["object_assignment"]).detach().cpu().numpy()
    pred_size_residual = torch.gather(pred_size_residual, 1, data_dict["object_assignment"].unsqueeze(2).repeat(1, 1,
                                                                                                                3)).detach().cpu().numpy()
...
def feed_scene_cap(model, device, dataset, dataloader, phase, folder, epoch_id,
                   use_tf=False, is_eval=True, max_len=CONF.TRAIN.MAX_DES_LEN, save_interm=False,
                   min_iou=CONF.EVAL.MIN_IOU_THRESHOLD, organized=SCANREFER_ORGANIZED, prepare_epochs=0,
                   samples_limit=None):
    candidates = {}
    intermediates = {}
    log = {
    ...
    if samples_limit is None:
        samples_limit = len(dataloader)
    for data_dict in tqdm(itertools.islice(dataloader, samples_limit), total=samples_limit):
        ...
        with torch.no_grad():
            ...
            data_dict = get_scene_cap_loss(data_dict, device, DC, weights=dataset.weights,
                                           prepare_epochs=prepare_epochs, detection=True, caption=False)

        _running_log = {}
        from lib.solver import extract_losses
        extract_losses(data_dict, _running_log)
        for key in _running_log:
            if key not in log:
                log[key] = []
            log[key].append(_running_log[key].cpu().numpy())
                ...
                if nms_masks[batch_id, prop_id] == 1 and good_bbox_masks[batch_id, prop_id] == 1:
                    ...
                    try:
                        ...
                        if save_interm:
                            ...
                            intermediates[scene_id][object_id]["box_corner"] = detected_bbox_corners[
                            ...
                            object_attn_weights = data_dict["topdown_attn"][:, :,
                                                  :num_proposals]  # NOTE only consider attention on objects
                            valid_context_masks = data_dict["valid_masks"][:, :,
                                                  :num_proposals]  # NOTE only consider attention on objects

                            cur_valid_context_masks = valid_context_masks[batch_id, prop_id]  # num_proposals
                            cur_context_box_corners = detected_bbox_corners[
                                batch_id, cur_valid_context_masks == 1]  # X, 8, 3
                            cur_object_attn_weights = object_attn_weights[
                                batch_id, prop_id, cur_valid_context_masks == 1]  # X

                            intermediates[scene_id][object_id][
                                "object_attn_weight"] = cur_object_attn_weights.cpu().numpy().T.tolist()
                            intermediates[scene_id][object_id][
                                "object_attn_context"] = cur_context_box_corners.cpu().numpy().tolist()
...
def eval_cap(model, device, dataset, dataloader, phase, folder, epoch_id,
             ...
             prepare_epochs=-1, samples_limit=None):
    if no_caption:
        bleu = 0
        cider = 0
        rouge = 0
        ...
        if not os.path.exists(corpus_path) or force:
            ...
            if dataset.name == "ScanRefer":
                raw_data = json.load(
                    open(os.path.join(CONF.PATH.SCANREFER, "ScanRefer_filtered_{}.json".format(phase))))

                scene_ids = set()
                for scr in dataset.scanrefer:
                    scene_ids.add(scr['scene_id'])

                raw_data = [r for r in raw_data if r['scene_id'] in scene_ids]

            elif dataset.name == "ReferIt3D":
                raw_data = json.load(open(os.path.join(CONF.PATH.SCANREFER, "nr3d_{}.json".format(phase))))
            else:
        ...
        if dataset.name == "ScanRefer":
            organized = json.load(open(os.path.join(CONF.PATH.SCANREFER, "ScanRefer_filtered_organized.json")))
        elif dataset.name == "ReferIt3D":
            organized = json.load(open(os.path.join(CONF.PATH.SCANREFER, "nr3d_organized.json")))
        else:
            raise ValueError("Invalid dataset.")
        ...
        if mode == "scene":
            candidates, log = feed_scene_cap(model, device, dataset, dataloader, phase, folder, epoch_id, use_tf,
                                             is_eval, max_len, save_interm, min_iou, organized=organized,
                                             prepare_epochs=prepare_epochs, samples_limit=samples_limit)
    ...
    if mode == "scene" or mode == "oracle":
        return bleu, cider, rouge, meteor, log
    else:
        return bleu, cider, rouge, meteor, cls_acc, log
